...
def main():
    ...
    if n_workers > 1 and len(tasks) > 1:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as ex:
            details = list(ex.map(mechanics_eval._evaluate_mechanics_for_sheet, tasks))
    else:
        details = [mechanics_eval._evaluate_mechanics_for_sheet(t) for t in tasks]
